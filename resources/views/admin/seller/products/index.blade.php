...
@section('content')
    <section>
        <div class="row">
            <div class="col-xs-12">
                <a href="{{route('product.create')}}" class="btn  btn-primary d-block" style="margin-bottom: 5px"><span class="fa fa-plus"></span> Add New Product </a>
                <div class="box">
                        ...
                        <table class="table table-hover">
                            ...
                            @forelse($products as $product)
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-danger" onclick="RemoveItem('item-{{$product->id}}')"> <i class="fa fa-trash"></i>   Remove </button>
                                        <a href="{{route('product.edit',$product->slug)}}" class="btn btn-sm btn-success"><i class="fa fa-edit"></i>  Edit </a>
                                        <a href="{{route('product.show',$product->slug)}}" class="btn btn-sm btn-warning"><i class="fa fa-eye"></i>  View </a>
                                    </td>
                                    <form action="{{route('product.destroy',$product->slug)}}" id="item-{{$product->id}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </tr>
